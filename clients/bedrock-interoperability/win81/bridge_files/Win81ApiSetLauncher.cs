...
public static class Win81ApiSetLauncherV16
{
    ...
    // Windows 8.1 API-set v4 stores namespace names without api-/ext- and without .dll.
    ...
    static void SetU32(byte[] b,int o,uint v){byte[] x=BitConverter.GetBytes(v);Buffer.BlockCopy(x,0,b,o,4);}
    ...
    static EntryRec Find(List<EntryRec> list,string name) {
        foreach(EntryRec e in list) if(String.Equals(e.Name,name,StringComparison.OrdinalIgnoreCase)) return e;
        return null;
    }
    ...
    static void SetDefaultHost(EntryRec e,string host) {
        if(e.Values.Count>0 && (e.Values[0].Name==null || e.Values[0].Name.Length==0)) {
    ...
    public static int Run(string exePath,string commandLine,string cwd) {
        ...
        if(!CreateProcessW(exePath,commandLine,IntPtr.Zero,IntPtr.Zero,false,createFlags,IntPtr.Zero,cwd,ref si,out pi)) throw new Exception("CreateProcessW failed: "+Marshal.GetLastWin32Error());
        Console.WriteLine("[INFO] Target CreateProcessW succeeded. PID={0}, TID={1}",pi.dwProcessId,pi.dwThreadId);
        bool resumed=false; bool detached=false;
        try {
            ...
            foreach(Target t in Targets) {
                ...
                if(e==null) {
                    ...
                    Console.WriteLine("[ADD] {0} -> {1}",t.Name,t.Host);
                    continue;
                }
                string h;
            ...
            Console.WriteLine("[OK] Self-check: all 17 target contracts are hosted; the 7 synthesized/repaired contracts use the selected compatibility hosts.");
            ...
            Console.WriteLine("[OK] Installed rebuilt, sorted private Win8.1 ApiSet v4 map ({0} bytes) for this Minecraft process only.",map.Length);

            if(!DebugActiveProcessStop(pi.dwProcessId)) throw new Exception("DebugActiveProcessStop failed: "+Marshal.GetLastWin32Error());
            detached=true;
            Console.WriteLine("[OK] IFEO recursion bypass detached; launcher authentication parameters are being forwarded unchanged.");
            uint rr=ResumeThread(pi.hThread); if(rr==0xFFFFFFFF) throw new Exception("ResumeThread failed: "+Marshal.GetLastWin32Error());
            resumed=true;
            Console.WriteLine("[OK] Minecraft resumed under the Win8.1 compatibility map; bridge will stay alive until the game exits.");
            uint wr=WaitForSingleObject(pi.hProcess,0xFFFFFFFF);
            if(wr!=0) throw new Exception("WaitForSingleObject failed/result: 0x"+wr.ToString("X8")+", last-error="+Marshal.GetLastWin32Error());
            uint ec; if(!GetExitCodeProcess(pi.hProcess,out ec)) throw new Exception("GetExitCodeProcess failed: "+Marshal.GetLastWin32Error());
            Console.WriteLine("[INFO] Minecraft process exit code: 0x"+ec.ToString("X8")+" ("+ec+")");
