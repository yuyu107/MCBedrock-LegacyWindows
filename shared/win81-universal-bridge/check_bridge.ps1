...
if($items.Count -eq 0){Write-Host '[INFO] No registered targets.'}else{foreach($k in $items){$v=Get-ItemProperty -LiteralPath $k.PSPath;$m=$v.Mode;if($m -eq 'interop'){$m='bedrock-interop (legacy alias: interop)'};Write-Host ('['+$m+'] '+$v.Path+'  ('+$v.RegisteredBy+')')}}
if($GameRoot){
  try{$current=[IO.Path]::GetFullPath((Join-Path ([IO.Path]::GetFullPath($GameRoot)) 'Minecraft.Windows.exe'));$found=$false;foreach($k in $items){$v=Get-ItemProperty -LiteralPath $k.PSPath;if([String]::Equals([IO.Path]::GetFullPath($v.Path),$current,[StringComparison]::OrdinalIgnoreCase)){$found=$true;$m=$v.Mode;if($m -eq 'interop'){$m='bedrock-interop (legacy alias: interop)'};Write-Host ('[CURRENT] '+$m+' -> '+$v.Path)}};if(-not $found){Write-Host ('[CURRENT] Not registered: '+$current)}}catch{}
}
